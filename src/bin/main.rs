...
pub async fn main() {
  ...
  static WEBSOCKET_HTML: &str = include_str!("websockets.html");

  // // The HTTP server handler
  // let http_handler = move |_: Request<()>, response: Response<()>| {
  //   // let mut response = response.expect(&"Could not start response");
  ...
  // GET / -> index html
  let index = warp::path::end().map(move|| warp::reply::html(WEBSOCKET_HTML));
  ...
  .map(|reply| {
    ...
    warp::reply::with_header(reply, "Sec-WebSocket-Protocol", "websocket-serial-json")
});
  ...
  warp::serve(routes).run(([127, 0, 0, 1], cfg.http_port)).await;
